...
// CI=0 หรือ CI>0
if($grade==0) {$cgrade = " WHERE ci_value = 0";}
else if($grade==1){$cgrade = " WHERE ci_value > 0";}
else{$cgrade="";}
...
// ความครอบคลุม
$strSQL = "CREATE TEMPORARY TABLE ci_0 AS SELECT ci_type, COUNT(ci_id) AS ci0 FROM ci_temp0 WHERE ci_value = 0";
if (strlen($town) == 4 ) {$strSQL = $strSQL." AND left(town_id, 4)='".$town."'";}
$strSQL = $strSQL." GROUP BY ci_type";
$objQuery = mysql_query($strSQL) or die ("Error Query [".$strSQL."]");

$strSQL = "CREATE TEMPORARY TABLE ci_1 AS SELECT ci_type, COUNT(ci_id) AS ci1 FROM ci_temp0 WHERE ci_value > 0";
if (strlen($town) == 4 ) {$strSQL = $strSQL." AND left(town_id, 4)='".$town."'";}
$strSQL = $strSQL." GROUP BY ci_type";
$objQuery = mysql_query($strSQL) or die ("Error Query [".$strSQL."]");
//

$strSQL = "CREATE TEMPORARY TABLE ci_temp AS SELECT * FROM ci_temp0". $cgrade;
if (strlen($town) == 4 ) {$strSQL = $strSQL." AND left(town_id, 4)='".$town."'";}
$objQuery = mysql_query($strSQL) or die ("Error Query [".$strSQL."]");
///////////////////////////////////////////////////////////////////////////////////////
...
if (strlen($town) == 4 ) {$strSQL = $strSQL." WHERE left(town_id, 4)='".$town."'";}
$strSQL = $strSQL." GROUP BY ci_type";
$objQuery = mysql_query($strSQL) or die ("Error Query [".$strSQL."]");
...
if ($grade==1) {$strSQL = "SELECT ci_title AS label, ci1 AS rightText, ci_type, nci0, nci1 FROM ci_rpt";}
else if($grade=="0") {$strSQL = "SELECT ci_title AS label, ci0 AS rightText, ci_type, nci0, nci1 FROM ci_rpt";}
else {$strSQL = "SELECT ci_title AS label, survey AS rightText, ci_type, nci0, nci1 FROM ci_rpt";}
$objQuery = mysql_query($strSQL) or die ("Error Query [".$strSQL."]");
